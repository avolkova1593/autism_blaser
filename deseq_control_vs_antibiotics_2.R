...
###Get deseq results
a_results <- results(a_processed, contrast = c("vector", "cont","antibiotics"))
f_results <- results(f_processed, contrast = c("vector", "cont", "antibiotics"))
###Select the genes with logfc > 1.5
a_results_logfc <- a_results[abs(a_results$log2FoldChange)>=1.5,]
f_results_logfc <- f_results[abs(f_results$log2FoldChange)>=1.5,]
...
f_plot_pca$Condition <- c(f_conditions)
